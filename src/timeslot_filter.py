...
def is_timeslot_valid(order: dict) -> bool:
    """
    True  -> поставку обрабатываем
    False -> пропускаем

    Правило:
    - если timeslot отсутствует -> пропускаем и пишем лог "Нет таймслота"
    - иначе берём local date в TIMEZONE и сравниваем с MIN_DATE (включительно)
    """
    try:
